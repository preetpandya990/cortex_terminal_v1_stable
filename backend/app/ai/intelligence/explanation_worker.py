...
class ExplanationOutput(BaseModel):
    ...
    suggested_action: str | None = Field(
        default=None,
        description=(
            "OPTIONAL — only populate when the prompt provides a "
            "'## Suggested Action Inputs' block. State a concrete next step "
            "using ONLY the exact numbers given there. Never invent a price, "
            "percentage, or date. If no such block is present, leave this "
            "field null — do not fabricate an action from other sections."
        ),
    )
...
# ── Suggested Action addendum (SUGGESTED_ACTION_ENABLED, learning-phase) ─────
#
# Appended to the base system prompts ONLY when the flag is on, composed at
# call time (not baked into the module-level constants above) so flipping the
# flag takes effect on the next job without a process restart. The field
# itself is optional on ExplanationOutput and only requested when a
# '## Suggested Action Inputs' block is actually present in the prompt — see
# _build_explanation_prompt / _build_context_prompt.
_SUGGESTED_ACTION_GUIDANCE = """

6. suggested_action (OPTIONAL FIELD — only when a '## Suggested Action Inputs' block is
   present in the prompt): state a concrete next step using ONLY the exact numbers in that
   block. Never invent a price, percentage, or date not given there. Still NEVER use
   absolute-certainty language ("guaranteed", "certain to", "will definitely") — describe
   what the system's own computed numbers suggest, not a prediction of what will happen.
   If no Suggested Action Inputs block is present, leave suggested_action null.\
"""

# ── Guardrail patterns ────────────────────────────────────────────────────────

# Guarantee/certainty alternatives factored out into their own pattern so both
# _strip_price_predictions (full_explanation — unchanged behavior) and the
# suggested_action guardrail (which must NOT block "target price"/"price
# target" — see _apply_suggested_action_guardrails) can share this part
# without either one weakening the other's rules.
_GUARANTEE_ONLY_RE = re.compile(
...
# Distinct sentinel (🧪, not ⚠) and materially different wording from
# _REGULATORY_DISCLAIMER — this is specifically about the suggested_action
# field's exploratory, learning-phase nature, not a generic informational-
# purposes notice. Fixed Python constant (never LLM-generated) so it can
# never be omitted by a bad generation. Appended to suggested_action only,
# never to full_explanation.
_SUGGESTED_ACTION_DISCLAIMER = (
    "\n\n🧪 This suggested action is generated by a system still in its "
    "learning phase and may be wrong, incomplete, or poorly calibrated. It is "
    "not a recommendation to buy, sell, or hold anything. Verify every number "
    "yourself and do your own independent research before acting."
)

# Deterministic fallback when every sentence of a generated suggested_action
# gets stripped by guardrails — never None/empty, so the frontend callout
# never renders blank.
_SUGGESTED_ACTION_FALLBACK_TEXT = (
    "No specific action could be grounded in verified data for this "
    "generation — review the signal details above and apply your own judgment."
)

# ₹ price figures — used only by the suggested_action grounding check (extends
# _strip_ungrounded_numbers' default %/RSI-only check for that field alone;
# full_explanation's grounding check is unchanged).
_PRICE_TOKEN_RE = re.compile(r"₹\s*(\d+(?:,\d{2,3})*(?:\.\d+)?)")
...
def _strip_ungrounded_numbers(
    text: str, prompt: str, extra_token_re: re.Pattern | None = None,
) -> tuple[str, int]:
    """
    ...
    ``extra_token_re``: an additional single-capture-group pattern to also
    ground-check (used by the suggested_action guardrail to cross-check ₹
    price figures via ``_PRICE_TOKEN_RE`` — full_explanation's check is
    unaffected since callers there never pass this).
    """
    removed = 0
    clean_lines: list[str] = []
    for line in text.split("\n"):
        ...
        for sentence in re.split(r"(?<=[.!?])\s+", line):
            ...
            for match in _GROUNDING_TOKEN_RE.finditer(sentence):
                ...
                if not any(v in prompt for v in _number_variants(num)):
                    grounded = False
                    break
            if grounded and extra_token_re is not None:
                for match in extra_token_re.finditer(sentence):
...
def _strip_guarantee_language(text: str) -> tuple[str, int]:
    """
    Remove sentences containing absolute guarantee/certainty language, using
    ONLY the guarantee/certainty alternatives (_GUARANTEE_ONLY_RE) — NOT the
    "target price"/"price target" phrase-block from _PRICE_PREDICTION_RE,
    which the suggested_action field is explicitly allowed to reference (the
    system's own already-computed exit/profit-booking level, not a
    prediction). Same per-line mechanics as ``_strip_price_predictions``.
    """
    removed = 0
    clean_lines: list[str] = []
    for line in text.split("\n"):
        ...
        kept: list[str] = []
        for sentence in re.split(r"(?<=[.!?])\s+", line):
            if _GUARANTEE_ONLY_RE.search(sentence):
                removed += 1
                logger.warning(
                    "explanation_worker: suggested-action guarantee guardrail "
                    "removed sentence: %.80s...",
                    sentence,
                )
            else:
                kept.append(sentence)
...
def _apply_suggested_action_guardrails(
    text: str | None, prompt: str,
) -> tuple[str | None, list[str]]:
    """
    Guardrails for the suggested_action field ONLY — a deliberately different
    pipeline from _strip_price_predictions (that guardrail blocks "target
    price"/"price target" outright, which this field must legitimately
    reference — see the prompt wording steering the model to say "exit
    level"/"profit-booking level" instead). Still blocks absolute
    guarantee/certainty language, and still grounds every ₹/%/RSI figure
    mentioned against the real numbers actually injected into the prompt.
    Never hard-fails: an all-stripped result returns the fixed fallback
    string, never None/empty, so the frontend callout never renders blank.
    """
    if not text:
        return None, []
    events: list[str] = []

    filtered, n = _strip_guarantee_language(text)
    events.extend(["suggested_action_guarantee_filter"] * n)

    filtered, n2 = _strip_ungrounded_numbers(filtered, prompt, extra_token_re=_PRICE_TOKEN_RE)
    events.extend(["suggested_action_ungrounded_filter"] * n2)

    if not filtered.strip():
        events.append("suggested_action_fallback_default")
        filtered = _SUGGESTED_ACTION_FALLBACK_TEXT
...
def _apply_guardrails(
    ...
    if output.suggested_action is not None:
        filtered_action, action_events = _apply_suggested_action_guardrails(
            output.suggested_action, prompt
        )
        events.extend(action_events)
        update["suggested_action"] = (
            filtered_action.rstrip() + _SUGGESTED_ACTION_DISCLAIMER
        )
...
# Short near-term reassessment window for the no-signal monitoring trigger —
# distinct from the 63-day vol lookback used elsewhere for actual SL/TP sizing.
_MOVE_THRESHOLD_MONITOR_DAYS = 5


def _compute_move_threshold_pct(
    annualized_volatility: float, monitor_days: int = _MOVE_THRESHOLD_MONITOR_DAYS,
) -> float:
    """
    Deterministic ±X% move-threshold for the no-signal suggested_action
    monitoring trigger — "how much movement over N days is notable given this
    instrument's own volatility," never a trade parameter. Same sqrt-of-time
    scaling convention as app/services/paper_trading/price_target_service.py
    (daily_vol = annualised_vol / sqrt(252)), scaled up to monitor_days
    instead of down to 1 day.
    """
    return annualized_volatility * math.sqrt(monitor_days / 252) * 100.0
...
def _build_explanation_prompt(
    ...
    if suggestion.take_profit_1:
        lines.append(f"Take Profit 1:    ₹{float(suggestion.take_profit_1):.2f}")
    ...
    if include_suggested_action and suggestion.entry_price and suggestion.stop_loss:
        lines.append("")
        lines.append("## Suggested Action Inputs")
        lines.append(f"Direction:      {suggestion.signal_direction}")
        lines.append(f"Entry Zone:     ₹{float(suggestion.entry_price):.2f}")
        lines.append(f"Stop Loss:      ₹{float(suggestion.stop_loss):.2f}")
        if suggestion.take_profit_1:
            lines.append(
                f"Exit/Profit-Booking Level 1: ₹{float(suggestion.take_profit_1):.2f}"
            )
        if suggestion.risk_reward_ratio:
            lines.append(f"Risk/Reward:    {float(suggestion.risk_reward_ratio):.1f}x")
        lines.append(
            "Using ONLY these numbers, state the suggested_action field: the "
            "direction, entry zone, stop-loss, and exit/profit-booking level, "
            "phrased as what the system's own signal parameters imply — never "
            "as a certainty. Do not use the words \"target price\" or \"price "
            "target\" — say \"exit level\" or \"profit-booking level\" instead."
        )
...
def _build_context_prompt(
    ...
    if current_price is not None and move_threshold_pct is not None and monitor_days is not None:
        lines.append("")
        lines.append("## Suggested Action Inputs")
        lines.append(f"Current Price:      ₹{current_price:.2f}")
        lines.append(f"Move Threshold:     ±{move_threshold_pct:.1f}%")
        lines.append(f"Timeframe:          {monitor_days}-day window")
        lines.append(f"Directional Lean:   {(ml_snapshot or {}).get('direction', 'N/A')}")
        lines.append(
            "Using ONLY these numbers, state the suggested_action field as a "
            "conditional monitoring trigger: \"if price moves ±{Move Threshold}% "
            "within {Timeframe}, that would reinforce/contradict the "
            "{Directional Lean} lean — worth checking back then.\" Never state "
            "an entry, stop-loss, or profit target — none exist for this "
            "instrument."
        )
...
async def _generate_explanation(
    ...
    *,
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Enrich the retrieval query with real company/sector signal instead
        # of bare ticker boilerplate — bare "{symbol} ... trading signal" gave
        # BM25/cosine nothing to discriminate topically relevant news from
        # generic high-buzz market wire content (see
        # NEWS_CONTEXT_RELEVANCE_GAP_FINDING.md). sector_peers feeds the
        # retriever's "sector" candidate tier so genuinely relevant
        # sector-adjacent company news can surface even with no
        # instrument-specific coverage.
        company_name = await symbol_validator.get_company_name(suggestion.symbol, db)
        sector, sector_peers = await resolve_sector_and_peers(
            db, suggestion.symbol, company_name, suggestion.instrument_key
        )
        query = " ".join(
            p for p in (suggestion.symbol, company_name, sector, suggestion.signal_direction, "trading signal") if p
        )

        try:
            chunks = await retrieve(
                db=db, query=query, symbol=suggestion.symbol, sector_peers=sector_peers,
            )
        except Exception as exc:
            logger.warning(
        ...
        prompt = _build_explanation_prompt(
            suggestion, context, price_action,
            include_suggested_action=settings.SUGGESTED_ACTION_ENABLED,
        )
    ...
    system_prompt = (
        _DEMAND_EXPLANATION_SYSTEM_PROMPT if is_demand else _EXPLANATION_SYSTEM_PROMPT
    )
    if settings.SUGGESTED_ACTION_ENABLED:
        system_prompt += _SUGGESTED_ACTION_GUIDANCE
    ...
    async with AsyncSessionLocal() as db:
        if final_output is not None:
            ...
            values: dict[str, Any] = dict(
                ...
                llm_suggested_action=final_output.suggested_action,
...
async def _generate_instrument_context(
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Enrich the retrieval query with real company/sector signal — see
        # the parallel comment in _generate_explanation for the rationale
        # (NEWS_CONTEXT_RELEVANCE_GAP_FINDING.md).
        company_name = await symbol_validator.get_company_name(eff_symbol, db)
        sector, sector_peers = await resolve_sector_and_peers(
            db, eff_symbol, company_name, instrument_key
        )
        query = " ".join(
            p for p in (eff_symbol, company_name, sector, "market analysis news") if p
        )
        try:
            chunks = await retrieve(
                db=db, query=query, symbol=eff_symbol, sector_peers=sector_peers,
            )
        except Exception as exc:
            logger.warning(
        ...
        # Best-effort enrichment for the no-signal monitoring trigger: current
        # price + a volatility-derived move threshold, both real numbers the
        # LLM restates verbatim (never invents) in suggested_action. Never
        # fails the whole generation over this optional enrichment.
        current_price: float | None = None
        move_threshold_pct: float | None = None
        if (
            settings.SUGGESTED_ACTION_ENABLED
            and ml_snapshot
            and ml_snapshot.get("available")
            and ml_snapshot.get("volatility") is not None
        ):
            try:
                candles = await RegimeService.load_ohlcv(db, instrument_key, limit=1)
                if candles:
                    current_price = float(candles[-1]["close"])
                    move_threshold_pct = _compute_move_threshold_pct(
                        float(ml_snapshot["volatility"])
                    )
            except Exception as exc:
                logger.warning(
                    "explanation_worker: current-price load failed for context %s "
                    "(suggested_action degrades to absent): %s",
                    instrument_key, exc,
                )

        prompt = _build_context_prompt(
            instrument_key, eff_symbol, ml_snapshot, context,
            current_price=current_price,
            move_threshold_pct=move_threshold_pct,
            monitor_days=(
                _MOVE_THRESHOLD_MONITOR_DAYS if move_threshold_pct is not None else None
            ),
        )
        prompt_hash = hashlib.sha256(prompt.encode()).hexdigest()
    # ── DB session closed ─────────────────────────────────────────────────────
    ...
    context_system_prompt = _CONTEXT_SYSTEM_PROMPT
    if settings.SUGGESTED_ACTION_ENABLED:
        context_system_prompt += _SUGGESTED_ACTION_GUIDANCE
