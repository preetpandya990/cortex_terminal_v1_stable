...
class FinalModelTrainer:
    ...
    async def load_best_hyperparameters(self) -> None:
        ...
        try:
            ...
            self.config.gru_best_params = {
                ...
                'clipnorm': 0.5,
                'clipvalue': None,  # Only use clipnorm (TF 2.21+ requirement)
    ...
    def _get_default_gru_params(self) -> Dict:
        ...
        return {
            ...
            'learning_rate': 0.001,
            'clipnorm': 0.5,
            'clipvalue': None,  # Only use clipnorm
            'patience': 15,
            'reduce_lr_patience': 5
        }
    ...
    async def generate_targets(self) -> None:
        """Generate target labels from database"""
        logger.info("\n" + "=" * 100)
        logger.info("TARGET GENERATION")
        logger.info("=" * 100)
        
        from app.ml.features.target_generator import create_targets_from_db
        
        # Generate targets directly from database (uses raw OHLCV data)
        ...
        # Validate alignment with features
        aligned_count = 0
        for symbol in list(self.targets_data.keys()):
            if symbol not in self.features_data:
                logger.warning(f"Target exists but no features for {symbol}, removing")
                del self.targets_data[symbol]
                continue
            
            # Align by trimming to minimum length
            feat_len = len(self.features_data[symbol]['X'])
            targ_len = len(self.targets_data[symbol]['target'])
            min_len = min(feat_len, targ_len)
            
            if feat_len != targ_len:
                logger.warning(f"{symbol}: Feature/target length mismatch ({feat_len} vs {targ_len}), trimming to {min_len}")
                self.features_data[symbol]['X'] = self.features_data[symbol]['X'][:min_len]
                self.features_data[symbol]['timestamps'] = self.features_data[symbol]['timestamps'][:min_len]
                self.targets_data[symbol]['target'] = self.targets_data[symbol]['target'][:min_len]
                self.targets_data[symbol]['timestamps'] = self.targets_data[symbol]['timestamps'][:min_len]
        ...
        logger.info(f"  Aligned symbols: {aligned_count}")
    ...
    async def train_xgboost(self) -> None:
        ...
        for symbol in self.symbols:
            if symbol in self.features_data and symbol in self.targets_data:
                ...
                if np.any(np.isnan(X_tab)) or np.any(np.isinf(X_tab)):
                    logger.warning(f"{symbol}: Invalid values in features, skipping")
                    continue
                if np.any(np.isnan(y)) or np.any(np.isinf(y)):
                    logger.warning(f"{symbol}: Invalid values in targets, skipping")
                    continue
        ...
        logger.info(f"Valid symbols: {len(X_list)}/{len(self.symbols)}")
        ...
        metrics_dict = asdict(results)
        # Convert numpy arrays to lists
        if 'confusion_matrix' in metrics_dict and hasattr(metrics_dict['confusion_matrix'], 'tolist'):
            metrics_dict['confusion_matrix'] = metrics_dict['confusion_matrix'].tolist()
    ...
    async def train_gru(self) -> None:
        ...
        if not X_list:
            raise ValueError("No valid data for GRU training")
        ...
        # Train (will use pre-built model and extract params from it)
        ...
        # Extract final epoch count safely
        try:
            final_epoch = len(history.history['loss']) if hasattr(history, 'history') else 0
        except (AttributeError, KeyError, TypeError):
            final_epoch = 0
            logger.warning("Could not extract final epoch count from history")
